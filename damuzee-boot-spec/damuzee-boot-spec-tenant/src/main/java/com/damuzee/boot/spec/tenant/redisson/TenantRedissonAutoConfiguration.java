...
/**
 * copy from {@link org.redisson.spring.starter.RedissonAutoConfiguration}
 */
@Configuration
@ConditionalOnClass({Redisson.class, RedisOperations.class})
@Import({RedisPoolAutoConfiguration.class})
@AutoConfigureAfter(RedisAutoConfiguration.class)//add this 避免覆盖lettuce/jedis
@ConditionalOnBean(RedisAutoConfiguration.class)
@EnableConfigurationProperties({RedissonProperties.class, RedisProperties.class})
public class TenantRedissonAutoConfiguration {
    ...
    private RedisProperties.Pool redisPool;

    //add this
    @Autowired(required = false)
    public void setRedisPool(RedisProperties.Pool pool) {
        if (Objects.nonNull(pool)) {
            redisPool = pool;
            return;
        }
        //自定义默认值
        redisPool = new RedisProperties.Pool();
        redisPool.setMaxIdle(8);
        redisPool.setMinIdle(8);
        redisPool.setMaxActive(8);
    }
    ...
    public RedissonClient redisson() {
        //add this
        int maxActive = redisPool.getMaxActive();
        int minIdle = redisPool.getMinIdle();
        ...
        if (redissonProperties.getConfig() != null) {
            ...
            config.useSentinelServers()
                ...
                .setSubscriptionConnectionPoolSize(maxActive)
